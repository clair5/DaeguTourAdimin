...
<body class="sb-nav-fixed">
	<div id="layoutSidenav">
		...
		<div id="layoutSidenav_content">
			...
			<div class="container px-4">
				...
				<div class="container px-4">
					<h1 class="mt-4" style="font-family: 'SBAggroB'">관광지 DB</h1>
					<ol class="breadcrumb mb-4">
						<li class="breadcrumb-item active" style="font-family: 'SBAggroB'">Beacon이
							등록된 관광지 목록</li>
					</ol>
					<div class="card mb-4">
						<div class="card-header" style="font-family: KIMM_Bold">
							<i class="fas fa-landmark"></i> 관광지 현황
						</div>
						<div class="card-body">
							<table
								...
								<thead class="bg-warning" style="font-family: KIMM_Bold">
									<tr>
										...
										<th scope="col" class="text-center"><i
											class="fas fa-tablet-alt"></i><a> </a>등록 기기</th>
									</tr>
								</thead>
								<tbody id="listArea" style="font-family: Pretendard-Regular">
									<c:forEach items="${list}" var="data">
										<tr>
											<td class="text-center"><a
												href="/view?la_nm=${data.la_nm}"
												style="text-decoration: none; color: black;">${data.la_nm}</a></td>
											<td class="text-center" id="la_mp">${data.la_mp}</td>
											<td class="text-center">${data.dev_ui}</td>
										</tr>
									</c:forEach>
							...
							<div class="btnList">
								<button type="button" class="writeBtn btn btn-warning"
									onclick="location.href='write'" style="float: right; font-family: Pretendard-Regular">
									<i class="fas fa-edit"></i><a> </a>신규 등록
								</button>
								<!-- 누르면 write.jsp 요청하고 controller는 write 메소드 실행해서 write.jsp 리턴-->
							</div>
						</div>
					</div>
					<div class="card mb-4">
	...
	<script>
		...
		var landName = [];
		<c:forEach items="${list}" var="data">
			positions.push("${data.la_mp}");
			landName.push("${data.la_nm}");
		</c:forEach>
		...
		// 주소로 좌표를 검색합니다
		  ...
		  // 인포윈도우로 장소에 대한 설명을 표시합니다
			        var infowindow = new kakao.maps.InfoWindow({
			            content: '<div style="width:150px;text-align:center;padding:6px 0;">' + landName[i] + '</div>'
			        });
